...
#R0_3 = R0_3.evalf(subs={q1: theta0, q2: t1, q3: t2+pi/2})

R3_6 = simplify(Transpose(R0_3))
R3_6 = R3_6 * RotM
R3_6 =  R3_6.evalf(subs = {q1:t0, q2:t1, q3:t2, roll_angle:, pitch_angle:pitch, yaw_angle:yaw})
r31 = R3_6[2,0]
r11 = R3_6[0,0]
r21 = R3_6[1,0]
r32 = R3_6[2,1]
...
///FIRST CALCULATE Q1-3///
///THEN FIND R3_6 AS OUTLINED IN THE LESSONS///

import numpy as np
import tf
import sympy

rotMatrix = R3_6.evalf(subs = {q1:q1_out, q2:q2_out, q3:q3_out, roll_angle:roll, pitch_angle:pitch, yaw_angle:yaw}) # rotMatrix is the solved R3_6 matrix, using actual values
# qx_out is q1-3 found earlier and passed to the arm
# roll/pitch/yaw_angle are the angles given in the initial request

rotMatrixNumpy = np.array(rotMatrix).astype(np.float64) # Has to be a numpy "matrix" instead of a sympy "Matrix" for the next function

alpha, beta, gamma = tf.transformations.euler_from_matrix(rotMatrixNumpy, axes = 'ryzx') # axes defines the rotation steps: r = rotating coordinate frame, then rotated by y,z,x
